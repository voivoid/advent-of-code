...
size_t run_parser( const std::string& line, const Parser& parser )
{
  ...
  const bool is_parsed                           = AoC::x3_parse( line, rule, x3::space, n );
  if ( !is_parsed )
  {
...
{
  return ranges::accumulate( ranges::getlines( input ) | ranges::views::transform( calc_encoded_line_len ), size_t{ 0 } );
}
